...
`ProjectManagerVisualization` now ships a repo-local Codex plugin so the
...
3. In Codex, install or enable the local plugin entry
   `ProjectManagerVisualization` from the repo marketplace.
...
1. Copy `plugins/project-manager-visualization/` to
   `~/plugins/project-manager-visualization/`.
2. Copy the marketplace entry into `~/.agents/plugins/marketplace.json` using
   the same `./plugins/project-manager-visualization` source path shape.
3. Install the plugin from your home-local marketplace.

## Typical Prompts

- `Use ProjectManagerVisualization to register the approved plan and build the reviewed Notion handoff.`
- `Use ProjectManagerVisualization to rescue this messy repo and make the true goal explicit before task generation.`
- `Use ProjectManagerVisualization to execute the next ready task and keep Notion current.`
