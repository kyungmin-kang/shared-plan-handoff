...
- When a phase becomes large, generate phase-scoped plan and task docs under `plans/<project_slug>/phase-docs/` and sync those copies into the Notion docs library too.
...
- Treat long Notion build/reconcile commands as progress-reporting operations; surface progress instead of leaving the operator guessing whether the build is hung.
